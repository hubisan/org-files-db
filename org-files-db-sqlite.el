;;; org-files-db-sqlite.el --- Interaction with the DB -*- lexical-binding: t -*-
...
(defun org-files-db--sqlite-open (file user-version schema-file)
...
build the database."
  (let* ((db-exists (org-files-db--sqlite-exists-p file))
         (db (when db-exists (sqlite-open file))))
    (unless (and db-exists (= user-version (org-files-db--sqlite-get-user-version db)))
      ;; Build or rebuid the database.
      (org-files-db--sqlite-build file user-version schema-file))
    (org-files-db--sqlite-turn-foreign-keys-on db)
    db))

(defun org-files-db--sqlite-close (db)
  "Close the DB."
  (sqlite-close db))

;;;; * Build

(defun org-files-db--sqlite-build (file user-version schema-file)
  "Build the database FILE using SCHEMA-FILE and set USER-VERSION.
Returns the sqlite database object of the created database."
  (org-files-db--sqlite-create-file file)
  (let ((db (sqlite-open file)))
    (org-files-db--sqlite-create-schema db schema-file)
    (org-files-db--sqlite-set-user-version db user-version)))

(defun org-files-db--sqlite-create-file (file)
...
(defun org-files-db--sqlite-exists-p (file)
