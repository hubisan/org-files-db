...
Describes the database tables used. For the actual schema, refer to the [[file:../sql/schema.sql][database schema file]]. Examples are provided to clarify column content where necessary.

** PRAGMA settings

- *user_version* (PRAGMA) The current schema version. Used internally to detect outdated databases. If the database version does not match, the existing database is deleted and rebuilt using.
- *foreign_keys* (PRAGMA) Foreign key constraints are enforced (`ON`).
...
- *created_at* (INTEGER) The timestamp of when the file record was created, in seconds since the epoch. Defaults to the current time when the record is created.
- *updated_at* (INTEGER) The timestamp of when the file record was last updated, in seconds since the epoch. Defaults to the current time when the record is updated.
...
#+BEGIN_QUOTE
Note:
A level 0 heading is included to represent the file-level scope. It allows file-wide elements such as keywords, file tags, and properties to be represented within the same hierarchical structure as regular headings.
#+END_QUOTE
...
- *outline* (TEXT) A JSON array representing the path from the top-level parent to the current heading, with each element as a heading title. (e.g., ["Top Parent", "Next \"Parent\"", "Current Heading"]).
- *all_tags* (TEXT) A JSON array representing all tags, direct and inherited ones. (e.g., ["work", "urgent", "accounting"]).
- *parent_id* (INTEGER) Foreign key referencing another entry in this table (the parent heading).

** tags

Each row represents a tag associated with a heading. File-level tags (FILETAGS) are also stored here, linked to the corresponding level 0 heading that represents the file itself.
...
Each row represents a file-level keyword extracted. These keywords (e.g., #+AUTHOR, #+TITLE, #+OPTIONS, #+EXPORT_FILE_NAME) define metadata or export-related settings that apply to the entire file. They are associated with the level 0 heading, which represents the file-level scope in the database schema.
...
#+BEGIN_QUOTE
Note:
- File-level properties defined with the property keyword (#+PROPERTY) are stored as properties of the level 0 heading.
- A category defined at the file level (#+CATEGORY) is stored as a property of the level 0 heading so that categories are treated uniformly from level 1 onward, simplifying database queries.
#+END_QUOTE
...
- *id* (INTEGER) Primary key.
- *heading_id* (INTEGER) Foreign key referencing the associated heading.
- *start_timestamp* (REAL) The start time in seconds since the epoch.
- *end_timestamp* (REAL) The end time in seconds since the epoch.
...
- *id* (INTEGER) Primary key
- *heading_id* (INTEGER) Foreign key referencing the associated heading. 
- *begin* (INTEGER) The point where the links starts in the file.
