...
- Adding to a value of an existing property is working as expected, even for inherited properties.
- Properties defined with a keyword at file level are added to properties as well. If the property already exists (defined as property) it is not overwritten.
- If the category is defined at file level is is added to properties as well unless it already exists (defined as property).
...
- *id* (INTEGER) Primary key.
- *heading_id* (INTEGER) Foreign key referencing the associated heading.
- *start_timestamp* (INTEGER) The start time in seconds since the epoch.
- *end_timestamp* (INTEGER) The end time in seconds since the epoch.
